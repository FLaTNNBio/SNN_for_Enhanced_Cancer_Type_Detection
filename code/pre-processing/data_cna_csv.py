...
def normalizza_e_concatena(input_csv_path, output_csv_path):
    ...
    # Seleziona le colonne numeriche a partire dalla seconda colonna
    colonne_numeriche = df.iloc[:, 2:].select_dtypes(include=[np.number])
    ...
    df_out = pd.concat([df_original.iloc[:, :2], normalized_columns], axis=1)
...
def elabora_sottocartelle(directory_principale):
    for root, dirs, files in os.walk(directory_principale):
        if 'data_cna.csv' in files:
            input_csv_path = os.path.join(root, 'data_cna.csv')
            output_csv_path = os.path.join(root, 'data_cna_normalized.csv')
